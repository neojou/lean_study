...
> 完整交接在 [`handoff.md`](handoff.md)。Phase 1 講義：[`lesson-1.md`](lesson-1.md)。Phase 2 講義：[`lesson-2.md`](lesson-2.md)。不要代寫作業。
> 日期：2026-09-09。
...
讀檔順序：[`handoff.md`](handoff.md) → 當前課程 [`lesson-1.md`](lesson-1.md) 或 [`lesson-2.md`](lesson-2.md) → 本檔。規格與全課綱不必一開場就讀完。
...
4. **模組與 namespace 用 `Mylogic`**（Lake `lake new mylogic` 的預設），不是舊草稿的 `MyLogic`。`import MyLogic.Formula` 是錯的。
...
## 與舊草稿的偏差（已寫進 `handoff.md`）

`handoff.md` 已於 2026-09-09 改寫成給**瀏覽器介面**的交接。下列以現況為準，不要再被更早的 `artifacts/mylogic` 文案帶跑：
...
| Phase 1／2 作法 | 直接寫 Lean 檔 | 講義 `lesson-1.md`、`lesson-2.md`，作者自己做作業 |
...
- Phase 2 作者在寫作業：看 [`lesson-2.md`](lesson-2.md) 附錄 A，不要直接給完整 `Deduction.lean`。
...
12. **離開 TUI 時的 WIP 已過時。** 作者後來寫完 `Formula.lean`，入口是 `import Mylogic.Formula`，`lake build` 通過。不要再把那份檔當成空註解。
13. **Phase 2 講義 `lesson-2.md`。** 與 Phase 1 同一原則：NJ 的 `Deduction` 由作者依 HW2.1–HW2.5 實作，模型不代寫。
...
Lake 模板的 `Mylogic/Basic.lean`（`hello`）與 `Main.lean` **Phase 0 已處理完**：保留模板；主入口不要依賴不存在的模組。Phase 1 才新增 `Mylogic/Formula.lean`。
...
作者若在做 Phase 2：指向 [`lesson-2.md`](lesson-2.md)，**批改或答疑，不要代寫 `Deduction.lean`**。作業通過後再把 [`TASKS.md`](TASKS.md) Phase 2 核取清單打勾。Phase 1 的 `Formula.lean` 已在 repo，不要重寫。若目標是改邏輯規格：先改 `first-order-logic.md`，再改 TASKS／講義，最後才動程式。
